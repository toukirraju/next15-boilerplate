...
import { deleteSubCategory, SubCategory, SubCategoryResponse } from '../actions/sub-categories';
...
const SubCategoryList = (categoryResponse: SubCategoryResponse) => {
    const [page, setPage] = useState(1);
    const router = useRouter();

    const { subCategories, pagination } = categoryResponse || {};
    ...
    const handleDeleteConfirmation = async () => {
        if (confirmationInput === deleteConfirmation.row?.name) {
            //delete category
            await deleteSubCategory(deleteConfirmation.row.id)
            setDeleteConfirmation({ isOpen: false, row: null });
            setConfirmationInput('');
        }
    };

    const handleEdit = (row: SubCategory) => {
        router.push(`/categories/edit?categoriId=${row.id}`);
    }
    const renderActions = (row: SubCategory) => {
        return (
            <Flex gap={'sm'}>
                <ActionIcon
                    color="blue"
